...
struct WeatherClient::Job {
 ...
 private:
  ...
  bool connectWifi();
  void releaseWifi();
  bool fetch(const char* url, std::string& out, size_t maxBytes, const char* what, cal_http::Stage* stage = nullptr);
...
bool WeatherClient::Job::fetchWeather(std::string& body, char* url, size_t urlSize) {
  ...
  for (int k = 0; k < n && !abandoned(); ++k) {
    ...
    bool parsed = false;
    auto attempt = [&](const char* what, size_t maxBytes, bool met) {
      cal_http::Stage st = cal_http::Stage::Ok;
      const bool got = fetch(url, body, maxBytes, what, &st);
    ...
    switch (r) {
      case Route::MetNo:
        ...
        attempt("погода (MET Norway)", kMaxMetBody, true);
        break;
      case Route::OpenMeteoHttps:
      ...
      case Route::OpenMeteoHttp:
        ...
        attempt("погода (Open-Meteo по HTTP)", kMaxBody, false);
        break;
    }
    if (!parsed) continue;
    w.atLat = target.lat;
...
bool WeatherClient::weatherDue(uint32_t nowEpoch) const {
  ...
  // MET Norway просит не спрашивать чаще, чем устаревает его ответ.
