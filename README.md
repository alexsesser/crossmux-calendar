...
|---|---|
| **Блок погоды** | «Погода»: страница 1 — ближайшие 24 часа (график температуры и осадков, ночь затенена, таблица каждые 3 часа с шапкой значками — t°, осадки, ветер — и **восход и закат**); страница 2 — 7 дней. Между страницами — свайп ←/→ или кнопки Left/Right. Внизу — кнопка **«Обновить»** (пока идёт загрузка — «Обновляю...»), время обновления и источник |
| **Число в сетке** | «День»: день недели, дата, «завтра / через N дн. / N дн. назад» (всегда отдельной строкой под датой), неделя и день года, **название праздника или переноса**, солнце (восход, закат, длина дня, разница со вчера, кривая за год), луна (фаза, освещённость, ближайшие полнолуние и новолуние), погода этого дня: на ближайшие 7 дней — прогноз, **для прошедших дней — история** (архив Open-Meteo; если архив из вашей сети недоступен — последний прогноз на этот день, сохранённый устройством; в заголовке карточки видно, что именно). Свайп ←/→ — ±1 день, ↑/↓ — ±1 неделя |
...
| **Название города** — на главном экране или в шапке «Погоды» | «Место»: откуда брать город — **автоматически по IP или вручную**, поиск города по названию (подробнее ниже) |
...
Экран «Место» открывается **тапом по названию города** — на главном экране (в блоке погоды) или в шапке экрана «Погода»:
...
- **«Координаты»** — ввести координаты (`55.75, 37.62`), затем подпись («Дача»; пусто — подписью станут сами координаты).
- **«Сохранённые»** — последние 5 выбранных вручную мест (найденных, введённых координатами, взятых «по IP») с подписями. Тап — выбрать, «x» справа — убрать из списка. Хранятся там же, в `calendar_settings.json`.
...
- **Запасные пути к погоде.** Серверы берутся только по именам через DNS вашей сети, адреса в прошивке не зашиты. У Open-Meteo несколько серверов, и DNS выбирает их по региону; из части домашних и мобильных сетей тот сервер, что DNS отдаёт, недоступен вовсе (журнал 26.09.2026: дома нет даже TCP-соединения с `api.open-meteo.com`, при этом соседний сервер Open-Meteo — поиск города — отвечает за 0,4 с; старая прошивка ведёт себя так же). Поэтому, если Open-Meteo по HTTPS не ответил, календарь пробует **тот же прогноз Open-Meteo под другими его адресами** — `previous-runs-api.open-meteo.com` и `historical-forecast-api.open-meteo.com` (`kOpenMeteoMirrorHosts`; их обслуживает другой сервер Open-Meteo — тот же, что отдаёт архив, — а данные те же до цифры), затем **основной сервер обычным HTTP** (если мешают только шифрованию; пропускается, когда сервер недоступен и по TCP), а затем **MET Norway** (`api.met.no`, Норвежский метеоинститут — другой сервер в другой сети; прогноз для всего мира). Сработавший путь запоминается и в следующий раз пробуется первым; запасной — не дольше часа (`kWeatherPrimaryRetryMin`), потом цепочка снова с Open-Meteo. Кнопка «Обновить» всегда проходит цепочку с начала. У MET Norway нет вероятности осадков для России — вместо «%» показываются миллиметры за час; погода от него обновляется не чаще раза в 30 минут (`kMetNoMinRefreshMin`, так просит сервис). Источник подписан внизу экрана «Погода». Выключить запасные пути — `kWeatherHttpFallback`, `kWeatherMetNoFallback`.
