...
- Раз в 5 минут (и не раньше, чем через ~4 с после входа на экран) устройство включает Wi-Fi, подключается к **последней сохранённой сети**, запрашивает данные и выключает Wi-Fi. Пока идёт запрос (несколько секунд) кнопки не откликаются, поэтому запрос **не начинается, пока вы пользуетесь экраном**: нужна пауза без ввода 3 секунды на главном экране и 10 секунд на вложенных (`kOnDemandDebounceMs`, `kDetailNetworkIdleSec`).
...
- Тап, пришедшийся на момент сетевого запроса (несколько секунд), теряется; запросы откладываются, пока идёт ввод, но полностью исключить это нельзя.
